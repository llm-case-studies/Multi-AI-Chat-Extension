...
chrome.storage.sync.get('webhookUrl', (data) => {
  ...
  if (!chatContainer) {
    console.log('Chat container not found. Update .conversation selector in deepseek_chat.js.');
    return;
  }
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === 1) {
            ...
            if (node.classList.contains('message-user')) {
              role = 'user';
            } else if (node.classList.contains('message-bot')) {
              role = 'assistant';
            }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'sendToAI' && (message.target === 'DeepSeek' || message.target === 'All')) {
